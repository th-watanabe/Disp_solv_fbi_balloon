...
!!!    real(kind=DP),    parameter :: beta = 6.d-2, dpdl =-1._DP
    real(kind=DP),    parameter :: dpdl =-1._DP

    real(kind=DP),    save :: k_perp, theta, beta
    ...
    public mag_response, mag_set_k_perp, mag_set_theta, mag_init, mag_set_beta
...
CONTAINS
  ...
  SUBROUTINE mag_set_beta( beta_new )

    real(kind=DP) :: beta_new

      beta = beta_new

  END SUBROUTINE mag_set_beta



  SUBROUTINE mag_response( z, zres )

    complex(kind=DP), intent(in)  :: z
    complex(kind=DP), intent(out) :: zres
